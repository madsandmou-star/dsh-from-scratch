...
try { await editTool.execute({ path: '../../etc/hosts', old_string: 'a', new_string: 'b' }, 不取消) }
catch (错误) { console.log(`  ❌ ${错误.message}`) }

console.log('\nbash 想删掉工作目录里的三年工作：')
console.log('  ' + await bashTool.execute({ command: 'rm -rf 重要资料 && echo 删完了', description: '清理' }, 不取消))

console.log('bash 想读工作目录之外的东西：')
console.log('  ' + await bashTool.execute({ command: 'head -1 /etc/hostname', description: '读系统文件' }, 不取消))
