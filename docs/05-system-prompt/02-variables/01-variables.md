...
## 教 debug：prompt 里出现了 `{{...}}` 或者少了一段

插值出问题有三种表现，各自看一个地方：

```sh
DSH_SHOW_PROMPT=1 npm run dev     # 段落清单 + 拼出来的全文
```

| 现象 | 根因 | 怎么确认 |
|---|---|---|
| prompt 里躺着一个 `{{cdw}}` | 这门课不会出现——未注册的变量会直接抛错 | 如果真看到了，说明那段文本没走 `interpolate()` |
| 启动就抛"引用了未注册的变量" | 段落里的名字和 `variable()` 注册的名字对不上 | 报错信息里就列着所有已注册的名字，比对一下 |
| 某一段莫名其妙变空了 | 取值函数返回了空串，或者段落本身求值成空 | 看**清单里的字符数**——全文里看不出少了什么，清单里 `0 字符` 一眼就看见 |

**清单比全文有用**，因为它把"每一段各贡献了多少"摊开了。全文只能告诉你结果，清单能告诉你是谁的锅。
