...
## 教 debug：模型好像没看见运行时上下文

快照走的是一条和 system prompt 完全不同的路径，所以查法也不同：**看模型这一轮实际收到的 messages**，而不是看 prompt。

```ts
console.dir(messages.slice(-3), { depth: null })   // 在每次 chatStream 之前
```

三种情况：

- **一条快照都没有** → `assembleContext()` 返回了空串。逐段注释掉再跑，看是哪一段求值成空的
- **有快照但内容是旧的** → 去重逻辑判定"和上次一样"了。打印 `lastSentSnapshot` 和这次算出来的，肉眼比对
- **回滚之后再也不发了** → 就是这一课那个 bug：`lastSentSnapshot` 和真实历史对不上了

第三条值得单独记：**当你发现"某个状态变量和真实数据对不上"时，先问它是不是本来就不该存在。** 这一课末尾讲的 dsh 解法就是把它删掉，改成从事件流里推导。
