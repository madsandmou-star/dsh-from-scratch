...
| 手法 | 什么时候用 |
|---|---|
| `DSH_SHOW_PROMPT=1 npm run dev`（5.1） | agent 行为不对——先看它到底收到了什么 system prompt |
| 看清单里的**字符数**而不是全文（5.2） | 某段话意外变空——全文里看不出少了什么，清单里 `0 字符` 一眼看见 |
| `console.dir(messages.slice(-3))`（5.3） | 模型好像没看见运行时上下文——它走的是 messages 不是 prompt |
| 逐段注释掉再跑（5.3） | 快照是空的——定位是哪一段求值成空 |

一条贯穿的规律：**这一阶段所有的 bug 都表现为"模型好像不知道某件事"，而查法永远是同一句——把模型这一轮实际收到的东西完整打出来。**
