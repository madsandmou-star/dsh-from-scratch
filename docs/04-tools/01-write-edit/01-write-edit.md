...
**② `content` 允许是空字符串，所以不能用 `requireString()`。** 清空一个文件是合法操作。3.3 那个校验函数把空串也拒了——那对 `path` 是对的，对 `content` 是错的。**同一个校验函数不一定适用于所有字段**，这是校验代码最常见的复制粘贴事故。
...
**为什么不直接 `content.replace(old_string, new_string)`？** 因为 `replace` 只换第一处却什么也不说——模型以为全换了，实际只改了一半。这是最难查的一类 bug：没有报错，结果是错的。
...
```
old_string 在 demo.ts 中出现了 2 次（第 2、6 line），必须恰好一次。
```
...
```
✅ write → 已创建 notes.md（4 ch）
✅ write → 已覆盖 notes.md（原 4 ch → 现 12 ch）
---
❌ edit → old_string 在 demo.ts 中出现了 2 次（第 2、6 line），必须恰好一次。请在 old_string 里多带上前后几行上下文，让它变得唯一。
❌ edit → old_string 在 demo.ts 中没有找到。请先用 read 确认原文（注意空格、缩进和换行必须完全一致）。
❌ edit → old_string 和 new_string 完全相同，这次替换不会改变任何东西。
✅ edit → 已修改 demo.ts（替换了 32 ch → 32 ch）
```
...
模型 >
  [工具] edit({"path":"demo.ts","old_string":"  return 1","new_string":"  return 2"})
         → error：old_string 在 demo.ts 中出现了 2 次（第 2、6 line），必须恰好一次。请在 old_string 里多带上前后几 …

模型 >
  [工具] edit({"path":"demo.ts","old_string":"export function b() {\n  return 1","new_string":"export function b() {\n  return 2"})
         → 已修改 demo.ts（替换了 32 ch → 32 ch）
...
```
【模型这一轮看到的 tool result】error：old_string 在 demo.ts 中出现了 2 次（第 2、6 line），必须恰好一次。请在 old_string 里多带上前后几行上下文，让它变得唯一。
【模型这一轮看到的 tool result】已修改 demo.ts（替换了 32 ch → 32 ch）
```
...
**它也报行号。** 我们上面那条改进不是自己想出来的，是照着它做的。找位置的循环（`matchOffsets`）和我们的 `findAllOffsets` 几乎逐行相同，包括 `offset = match + search.length` 那个防重叠的写法。
