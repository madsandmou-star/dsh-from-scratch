...
| 手法 | 什么时候用 |
|---|---|
| 每条命令前加 `pwd &&`（4.2） | agent 的命令莫名找不到文件——它以为自己在别的目录 |
| 先证明"我走到那个文件了"（4.3） | 搜索结果为空——分清"没找到"和"没搜到" |
| 插几层只打印的 trace 护栏（4.4） | 不知道是哪一层改了结果，或者哪一层拒绝了 |
| 看模型收到的 tool 结果，而不是看你的代码（4.1） | agent 改错了文件——它的下一步完全由它看到的东西决定 |

最后一条是这一阶段的总纲：**工具的读者是模型。要 debug 它的行为，先看它读到了什么。**
