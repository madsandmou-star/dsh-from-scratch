...
| 手法 | 什么时候用 |
|---|---|
| `| cat` 关掉管道缓冲（2.1） | 觉得"流式没生效"时——很可能是缓冲在骗你 |
| 把每个原始 chunk 连长度一起打出来（2.2） | 分帧出问题时。网络 chunk 边界是随机的，肉眼看不出来 |
| 打印每条事件的到达时刻（2.4b） | 分辨"被截断"、"对端挂住"和"max_tokens 太小" |
| 诊断信息走 `stderr`（2.3） | 流式输出正在占用 stdout，混在一起就看不清了 |

最后一条容易忘：**流式的时候，stdout 是给用户的，stderr 才是给你的。**
