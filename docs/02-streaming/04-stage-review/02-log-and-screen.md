...
这一节讲的三层（内存日志、广播、落盘）出问题时，症状都长得像"UI 显示不对"，但根因在三个完全不同的地方。分辨方法是**逐层往下问**：

| 现象 | 先看哪一层 | 怎么看 |
|---|---|---|
| 屏幕上有，重开会话就没了 | **落盘** | 去日志文件里 grep 那段文本；找不到就是没写进去 |
| 日志里有，屏幕上没有 | **广播/订阅** | 看那个订阅者是不是在事件发生之后才注册的 |
| 两边都有但顺序不对 | **append 的调用点** | 打印每条事件的 `seq`，看追加顺序和你以为的是否一致 |

**先确定"谁是权威"，再顺着它往下查。** 在这一层里权威是那条只增不改的日志——屏幕和文件都只是它的投影，投影出错永远比源头出错更常见。
