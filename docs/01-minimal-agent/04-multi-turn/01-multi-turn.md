...
我们的 `messages` 同时是三样东西：**发给模型的请求内容**、**给用户显示的对话**、以及**"发生过什么"的记录**。三个角色被同一个可变数组承担，所以谁改了它，另外两个都跟着变——上面那个 `while (messages.length > rollbackTo) pop()` 就是一次性抹掉了三样东西。

dsh 把这三件事拆开了：

| | 我们的 | dsh 的 | 为什么 dsh 更复杂 |
|---|---|---|---|
| 权威数据 | `messages` 数组（可变） | 一条**只增不改**的事件日志（`dsh/packages/core/session/src/index.ts`） | 只增不改才能重放、才能审计、才能崩溃后恢复 |
| 发给模型的内容 | 就是那个数组 | 从日志**投影**出来 | 投影规则可以变（压缩、隐藏、替换），日志不用动 |
| 给用户显示的 | 就是那个数组 | 也从日志投影，但投影方式不同 | 界面要显示工具卡片、折叠快照，模型不需要这些 |
| 撤回一条 | `pop()` | 追加一条"这条不算数了"的事件 | 历史不可改，所以"撤销"本身也是一条历史 |

阶段 6 会把这三个角色拆开——那时候 `messages` 数组本身就退位了。
